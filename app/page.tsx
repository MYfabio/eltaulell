  ...
  return (
    <main>
      ...
      <section className="hero">
        ...
        <div className="actions"><Link className="button" href="/taulell">Entrar al taulell</Link><a className="button secondary" href="#calendari">Veure el calendari</a></div>
      </section>
      <section className="cards">
        {cards.map(([title, description], index) => <article key={title}><h2>{title}</h2><p>{description}</p>{index === 0 ? <Link href="/taulell">Obrir el suro →</Link> : <span>Properament</span>}</article>)}
      </section>
